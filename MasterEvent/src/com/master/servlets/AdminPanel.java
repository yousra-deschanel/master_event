...
import com.master.dao.AdminDao;
...
public class AdminPanel extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		this.getServletContext().getRequestDispatcher("/WEB-INF/AdminLog.jsp").forward(request, response);
	}
	...
	private void authenticate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (AdminDao.validateAdmin(loginBean)) {
				RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/Admin.jsp");
				dispatcher.forward(request, response);
			} else {
				response.getWriter().append("invalid email or password for the Admin").append(request.getContextPath());
